...
import (
	...
	"github.com/gfscore/go-gfscore/common/fdlimit"
...
func main() {
	...
	for _, node := range nodes {
		var gfscore *eth.Ethereum
		if err := node.Service(&gfscore); err != nil {
			panic(err)
		}
		if err := gfscore.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var gfscore *eth.Ethereum
		if err := nodes[index%len(nodes)].Service(&gfscore); err != nil {
			panic(err)
		}
		...
		if err := gfscore.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := gfscore.TxPool().Stats(); pend > 2048 {
