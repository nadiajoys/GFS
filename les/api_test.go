...
import (
	...
	"github.com/gfscore/go-gfscore/p2p/simulations"
	"github.com/gfscore/go-gfscore/p2p/simulations/adapters"
...
func newLesServerService(ctx *adapters.ServiceContext) (node.Service, error) {
	...
	gfscore, err := eth.New(ctx.NodeContext, &config)
	if err != nil {
		return nil, err
	}
	server, err := NewLesServer(gfscore, &config)
	if err != nil {
		return nil, err
	}
	gfscore.AddLesServer(server)
	return gfscore, nil
}
