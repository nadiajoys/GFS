...
import (
	...
	"github.com/gfscore/go-gfscore/gfs/fetcher"
...
// NewProtocolManager returns a new Gfscore sub protocol manager. The Gfscore sub protocol manages peers capable
// with the Gfscore network.
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb gfsdb.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Gfscore peer", "peer", id)

	// Unregister the peer from the downloader and Gfscore peer set
...
func (pm *ProtocolManager) Start(maxPeers int) {
	...
	go pm.txsyncLoop64() // TODO(karalabe): Legacy initial tx echange, drop with gfs/64.
}
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Gfscore protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an gfs peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Gfscore peer connected", "name", p.Name())

	// Execute the Gfscore handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkid.NewID(pm.blockchain), pm.forkFilter); err != nil {
		p.Log().Debug("Gfscore handshake failed", "err", err)
		return err
	}

	// Register the peer locally
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Gfscore peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Gfscore message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Gfscore sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Gfscore network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
